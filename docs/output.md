...
| `candidate`          | Set **alongside** an MoI tag when family structure cannot prove segregation. Fires only for: (a) `recessive` in non-trio families, (b) `x_recessive` in non-trio families _except_ duo-with-mom for a male proband. Non-trio `slivar_comphet`s are also unprovable but the `candidate` flag is **not** auto-applied to them (would require a post-compoundhets pass) — treat any `slivar_comphet` in a non-trio family as a candidate by convention. | Non-trio families with an unprovable single-variant MoI                                                                                                                         |
...
| Duo with unaffected parent          | `recessive` + `candidate` when proband hom-alt + parent het. `x_recessive` confirmed (no `candidate`) when mom + male proband. `slivar_comphet` for paired hets (treat as candidate by convention since family is non-trio). `denovo`/`dominant` do not fire (no segregation evidence). |

...
- [#106](https://github.com/Ferlab-Ste-Justine/Post-processing-Pipeline/pull/106) Allow `familyId` and `sample` to be specified as integers in the samplesheet, not only strings.

### `Added`

- [#XXX](https://github.com/Ferlab-Ste-Justine/Post-processing-Pipeline/pull/XXX) Add parental origin tagging for trios in the slivar inheritance step. Genotype ploidy is now normalized after variant normalization (new `BCFTOOLS_PLUGINFIXPLOIDY` module) so DRAGEN-style haploid X/Y calls don't collapse to `UNKNOWN` in slivar.

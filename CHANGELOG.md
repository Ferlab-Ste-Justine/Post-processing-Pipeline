...
- [#XXX](https://github.com/Ferlab-Ste-Justine/Post-processing-Pipeline/pull/XXX) **Breaking:** BIOINFO-222: Renamed the `exclude_mnps` parameter to `gvcf_filtering`, and the `EXCLUDE_MNPS` subworkflow to `SANITIZE_GVCF_RECORDS`. The old names described removing MNPs, but that filter never actually matched anything (see `Fixed` below) — the step's real, working job has always been sanitizing malformed/duplicate gVCF records, which the new names describe accurately. Update any `params.json`/`params.yaml` using `exclude_mnps` to use `gvcf_filtering` instead.

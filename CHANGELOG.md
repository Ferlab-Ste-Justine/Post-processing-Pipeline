...
- [#XXX](https://github.com/Ferlab-Ste-Justine/Post-processing-Pipeline/pull/XXX) BIOINFO-232: `EXOMISER` now uses the official `exomiser/exomiser-cli:14.0.0-bash` image instead of the custom `ferlabcrsj/exomiser` one. The exomiser version reported in `versions.yml` is now read from the image's `exomiser-core` jar filename instead of a baked-in `/EXOMISER_VERSION.txt`, and the image's `/bin/bash` entrypoint is cleared for `EXOMISER` in the `docker` and `podman` profiles.
- [#XXX](https://github.com/Ferlab-Ste-Justine/Post-processing-Pipeline/pull/XXX) BIOINFO-232: Removed the `containers/` directory (custom `exomiser` and `exomiser-13` Dockerfiles) and the three `publish_with_*.yml` workflows that built and pushed them to `ferlabcrsj/exomiser`. Images already on Docker Hub are unaffected. Custom images, if needed again, will be maintained in a dedicated repository rather than in a pipeline repository.

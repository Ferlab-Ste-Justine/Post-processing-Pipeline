...
- [#XXX](https://github.com/Ferlab-Ste-Justine/Post-processing-Pipeline/pull/XXX) `EXOMISER` now uses the official `exomiser/exomiser-cli:14.0.0-bash` image instead of the custom `ferlabcrsj/exomiser` one. The exomiser version reported in `versions.yml` is now read from the image's `exomiser-core` jar filename instead of a baked-in `/EXOMISER_VERSION.txt`, and the image's `/bin/bash` entrypoint is cleared for `EXOMISER` in the `docker` and `podman` profiles.
